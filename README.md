...
you can call the core `analog_search()` function. Common query types are
supported via dedicated helper functions:

- `analog_velocity()` finds the k nearest geographic analogs under a
  hard climate constraint
- `analog_impact()` finds the k nearest climate analogs within a
