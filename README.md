...
The core engine is `analog_search()`, which exposes the full flexibility
of the framework. Simplified wrapper functions configure it for common
analysis types:

- `analog_impact()` predicts ecological state variables using analog
  impact models (AIMs)
- `analog_regression()` fits local weighted regressions across analog
  neighborhoods
- `analog_velocity()` finds the nearest geographic analogs under a
  climate constraint
- `analog_similarity()` finds the nearest climate analogs within a
