...
test_that("analog_search works for all modes with new architecture", {
      ...
      # knn_geog
      v <- analog_search(d$focal, d$ref, mode = "knn_geog", max_clim = 1, k = 1,
                        coord_type = "projected", index_res = 10)
      expect_equal(nrow(v), nrow(d$focal))

      # knn_clim
      i <- analog_search(d$focal, d$ref, mode = "knn_clim", max_geog = 2, k = 3,
                        coord_type = "projected", index_res = 10)
      expect_true(nrow(i) <= nrow(d$focal) * 3)

      # count
      c <- analog_search(d$focal, d$ref, mode = "count", max_clim = 1,
                        coord_type = "projected", index_res = 10)
      expect_equal(nrow(c), nrow(d$focal))

      # sum
      s <- analog_search(d$focal, d$ref, mode = "sum", max_clim = 1,
                        weight = "uniform", coord_type = "projected", index_res = 10)
      expect_equal(nrow(s), nrow(d$focal))

      # all
      a <- analog_search(d$focal, d$ref, mode = "all", max_clim = 1,
                        coord_type = "projected", index_res = 10)
...
test_that("analog_search with index works for different modes", {
      ...
      # knn_geog
      v <- analog_search(d$focal, index, mode = "knn_geog", max_clim = 1, k = 1)
      expect_equal(nrow(v), nrow(d$focal))
      expect_true(all(c("focal_index", "analog_index", "geog_dist") %in% names(v)))

      # knn_clim
      i <- analog_search(d$focal, index, mode = "knn_clim", max_geog = 2, k = 3)
      expect_true(nrow(i) <= nrow(d$focal) * 3)

      # count
      c <- analog_search(d$focal, index, mode = "count", max_clim = 1, max_geog = 2)
      expect_equal(nrow(c), nrow(d$focal))
      expect_true("value" %in% names(c))
      expect_true(all(c$value >= 0))

      # sum
      s <- analog_search(d$focal, index, mode = "sum", max_clim = 1, max_geog = 2,
                        weight = "uniform")
      expect_equal(nrow(s), nrow(d$focal))

      # all
      a <- analog_search(d$focal, index, mode = "all", max_clim = 1, max_geog = 2)
      expect_true(nrow(a) >= nrow(d$focal))
})
...
test_that("analog_search with index validates inputs", {
      ...
      expect_error(
            analog_search(bad_focal, index, mode = "count"),
            "has 5 columns but index expects 4"
      )
})


test_that("analog_search preserves backward compatibility with raw data", {
...
test_that("analog_search index path handles all constraint combinations", {
      ...
      r1 <- analog_search(d$focal, index, mode = "count", max_clim = 1)
      expect_true(all(r1$value >= 0))

      # Only geographic constraint
      r2 <- analog_search(d$focal, index, mode = "count", max_geog = 2)
      expect_true(all(r2$value >= 0))

      # Both constraints
      r3 <- analog_search(d$focal, index, mode = "count", max_clim = 1, max_geog = 2)
      ...
      r4 <- analog_search(d$focal, index, mode = "count")
