...
#   - Returns a list with numeric elements `geog_res_adj` and `clim_res_adj`.
...
expect_adj_list <- function(res) {
      ...
      expect_named(res, c("geog_res_adj", "clim_res_adj"), ignore.order = TRUE)
      expect_true(is.numeric(res$geog_res_adj) && length(res$geog_res_adj) == 1L)
      expect_true(is.numeric(res$clim_res_adj) && length(res$clim_res_adj) == 1L)
}
...
test_that("tune_index_res returns a valid per-family list", {
      ...
      expect_true(res$clim_res_adj > 0)
})
...
test_that("tune_index_res passes inputs through unchanged on small data", {
      ...
      expect_equal(res$clim_res_adj, 0.5)
})
...
test_that("tune_index_res handles tiny datasets (pass-through)", {
      ...
      expect_equal(res$clim_res_adj, 2)
})
...
test_that("tune_index_res tunes large datasets within bounds", {
      ...
      expect_true(res$clim_res_adj >= ADJ_MIN && res$clim_res_adj <= ADJ_MAX)
})
...
test_that("tune_index_res keeps tuned adjustments in [1/32, 32]", {
      ...
      expect_adj_list(res)
      # knn_geog constrains climate (filter) and sorts geographically, so both
...
test_that("tune_index_res passes a deactivated family through untouched", {
      ...
      expect_adj_list(res)
      expect_equal(res$clim_res_adj, 0)                 # untouched
      ...
      expect_true(res2$clim_res_adj >= ADJ_MIN &&
                        res2$clim_res_adj <= ADJ_MAX)         # tuned, in bounds
})
...
test_that("tune_index_res returns inputs when both families deactivated", {
      ...
      res <- tune_index_res(
            ...
            stat = "count",
            coord_type = "projected",
            geog_res_adj = 0, clim_res_adj = 0,
            verbose = FALSE
      )
      expect_adj_list(res)
      expect_equal(res$geog_res_adj, 0)
      expect_equal(res$clim_res_adj, 0)
})
