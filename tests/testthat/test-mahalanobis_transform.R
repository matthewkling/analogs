test_that("mahalanobis_transform works with single matrix input", {
      ...
      set.seed(123)
      # Create correlated climate data
      n <- 100
...
test_that("mahalanobis_transform works with paired inputs", {
      ...
      x_clim <- matrix(rnorm(n_x * 2), ncol = 2)
      x_clim[, 2] <- x_clim[, 2] + 0.7 * x_clim[, 1]
      x_data <- cbind(matrix(rnorm(n_x * 2), ncol = 2), x_clim)

      pool_clim <- matrix(rnorm(n_pool * 2), ncol = 2)
      pool_clim[, 2] <- pool_clim[, 2] + 0.7 * pool_clim[, 1]
      pool_data <- cbind(matrix(rnorm(n_pool * 2), ncol = 2), pool_clim)
...
test_that("mahalanobis_transform respects center and scale parameters", {
      ...
      x_clim <- matrix(c(rnorm(n, mean = 10, sd = 2),
                         rnorm(n, mean = 1000, sd = 500)), ncol = 2)
      x_data <- cbind(matrix(rnorm(n * 2), ncol = 2), x_clim)
...
test_that("mahalanobis_transform validates input dimensions", {
      ...
      expect_error(
            ...
            "same number of climate variables"
      )
})
...
test_that("mahalanobis_transform handles data.frame input", {
      ...
      set.seed(222)
      n <- 50
      x_clim <- matrix(rnorm(n * 2), ncol = 2)
      x_df <- data.frame(
            x = rnorm(n),
            y = rnorm(n),
            temp = x_clim[, 1],
...
test_that("transformation actually enables Mahalanobis distance", {
      ...
      cov_transformed <- cov(transformed_clim)
      ...
      p1_orig <- x_clim[1, ]
      p2_orig <- x_clim[2, ]
      p1_transformed <- transformed_clim[1, ]
      p2_transformed <- transformed_clim[2, ]
...
test_that("mahalanobis_transform detects zero variance", {
      ...
      # Second climate variable has zero variance
      x_clim <- cbind(rnorm(n), rep(5, n))
      x_data <- cbind(matrix(rnorm(n * 2), ncol = 2), x_clim)
