...
1. **Select** a neighborhood of analog locations from a reference pool, based on climate similarity, geographic proximity, or both.
2. **Summarize** the neighborhood using counts, weighted means, regression coefficients, or other statistics.

Common methods in climate change ecology---climate velocity, analog impact models, analog availability---are all specific configurations of this framework. So are geographically weighted regression (GWR) and inverse distance-weighted (IDW) interpolation, which use purely geographic neighborhoods. The package provides a flexible core function, `analog_search()`, along with simplified wrappers for common analysis types.
