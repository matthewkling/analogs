...
#' (`theta`). Pass one to the `env` argument of [analog_search()] and/or one
...
#'   threshold. Usually a single radius. For the environmental family, `max` may also
#'   be a vector of per-variable absolute-difference thresholds (length equal to
#'   the number of environmental variables); the geographic family uses a single
#'   radius. Supplied to the search as `max_env` / `max_geog`.
#'
